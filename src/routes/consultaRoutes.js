...
// aplicando as barreiras de segurança (middleware) nas rotas de consulta
...
router.get('/', autenticarToken, listAll);

// Rota POST com middleware de validação Zod
router.post('/', autenticarToken, autorizarCargos( 'ADMIN', 'MEDICO'), async (req, res, next) => {
  try {
...
router.get('/:id', autenticarToken, async (req, res, next) => {
  try {
